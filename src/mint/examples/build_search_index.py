...
def main():
    parser = argparse.ArgumentParser(
        description="Build a search index using BERT (or SentenceBERT)"
    )
    parser.add_argument("--input", help="Input file")
    parser.add_argument(
        "--index",
        help="Index name.  This will yield an NPZ and a FAISS index file",
        default="search",
    )
    ...
    with torch.no_grad():
        for (batch, texts) in read_batch(
            ...
            embedded_batch = (
                embedder(batch, embedder.create_pad_mask(batch)).cpu().numpy()
            )
