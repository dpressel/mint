...
"""Pre-train a T5 model in PyTorch (Simple single file version)
...
def create_single_file_dataset(tokenizer: Tokenizer, fname: str, seq_len: int = 512) -> Dataset:
    ...
    with open(fname) as rf:
        ...
        num_toks = seq_len - 1  # Ignore CLS and SEP
        num_samples = len(tokens) // num_toks * num_toks
        tensors = [tokens[i : i + num_toks] + [eos_token] for i in range(0, num_samples, num_toks)]
...
def main():
    parser = argparse.ArgumentParser(description='Pretrain T5 (simple)')
    ...
    parser.add_argument("--tok_file", type=str, help="The path to the GPT2 tokenizer", required=True)
    ...
    if args.restart_from:
        ...
        model = T5Creator.from_pretrained(args.restart_from, **vars(args))
    else:
        global_step = 0
        model = T5SequenceGenerator(tokenizer.get_vocab_size(), **vars(args))
    print(model)
