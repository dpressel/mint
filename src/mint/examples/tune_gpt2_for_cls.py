...
def main():
    parser = argparse.ArgumentParser(
        description="fine-tune GPT for classification (single text input only)"
    )
    ...
    parser.add_argument(
        ...
        type=str,
        choices=["mean", "last"],
        help="token to use for pooling, defaults to mean pooling",
    )
    ...
    pool_id = (
        tokenizer.token_to_id("<|endoftext|>") if args.pool_type == "last" else None
    )
    model = GPT2Creator.pooled_enc_from_pretrained(
        args.model, output=output_layer, pool_id=pool_id, **vars(args)
    ).to(args.device)
