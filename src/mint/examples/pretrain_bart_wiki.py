...
"""Pre-train a BART model in PyTorch on all of wikipedia via https://github.com/attardi/wikiextractor
...
def main():
    parser = argparse.ArgumentParser(description='Pretrain BART (wiki)')
    ...
    parser.add_argument("--tok_file", type=str, help="The Tokenizer file or model dir", required=True)
    ...
    Trainer = DistributedSeq2SeqTrainer if args.distributed else SingleDeviceSeq2SeqTrainer
    trainer = Trainer(
        model,
        global_step=global_step,
        collate_function=NoisingCollator(tokenizer.get_vocab()),
        **vars(args),
    )
    logger.info(trainer)
